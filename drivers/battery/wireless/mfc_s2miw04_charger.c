...
static void mfc_set_tx_fod_common(struct mfc_charger_data *charger)
{
	...
	/* Thresh1 */
	mfc_reg_write(charger->client, MFC_TX_FOD_THRESH1_L_REG, 0xD6);	/* 0x06D6: 1750mW */
	mfc_reg_write(charger->client, MFC_TX_FOD_THRESH1_H_REG, 0x6);
	/* Thresh2 */
	mfc_reg_write(charger->client, MFC_TX_FOD_THRESH2_REG, 0x0);
}
	...
	case POWER_SUPPLY_PROP_ENERGY_NOW:
		...
		if (charger->wc_tx_enable) {
			...
			pr_info("@Tx_Mode %s Firmware Ver = %x TX_VOUT = %dmV, TX_IOUT = %dmA, OP_FREQ = %dKHz, TX_OP_FREQ = %dKHZ, %s connected\n",
				__func__, charger->pdata->otp_firmware_ver, vout, iout, freq,
				tx_freq, rx_device_type_str[charger->wc_rx_type]);
